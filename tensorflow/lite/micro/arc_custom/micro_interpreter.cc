...
#define DEBUG_IDX_DUMP
#if defined(DEBUG_IDX_DUMP)
#include "tensorflow/lite/micro/kernels/kernel_util.h"
#include "tensorflow/lite/micro/idx_file.h"
#include "string.h"
#endif
namespace tflite {
namespace {
...
#if defined(DEBUG_IDX_DUMP)
void DebugWriteResultToFile(TfLiteContext* context, char* filename,
                            TfLiteEvalTensor* tensor) {
  uint32_t arr_size = tflite::micro::GetTensorShape(tensor).FlatSize();
  tIdxDataType d_type = IDX_DT_BYTE_1B;
  if (tensor->type != kTfLiteInt8) {
    switch (tensor->type) {
      case kTfLiteFloat32: 
        d_type = IDX_DT_FLOAT_4B; break;
      case kTfLiteInt32: 
        d_type = IDX_DT_INT_4B; break;
      default:
        context->ReportError(context, "Unsopported type tensor");
        return;
    }
  }

  tIdxRetVal ret_status = idx_file_write_completely(
      filename, tflite::micro::GetTensorData<void>(tensor), &arr_size, 1,
      d_type);
  if (ret_status != IDX_ERR_NONE)
    context->ReportError(context, "TENSOR FILE WRITE FAILED");
}
#endif
}  // namespace
...
#if defined(DEBUG_IDX_DUMP)
    for (int out_idx = 0; out_idx < node->outputs->size; ++out_idx) {
      static char out_path[40] = {0};
      const char* zeroes_str = (i >= 10) ? "0" : "00";
      if (i >= 100) zeroes_str = "";


      sprintf(out_path, "NODE_%s%d_OUT_%d_%s.idx", zeroes_str, i, out_idx, 
              OpNameFromRegistration(registration));
      DebugWriteResultToFile(
          &context_, out_path,
          tflite::micro::GetEvalOutput(&context_, node, out_idx)
                             );
    }
#endif
