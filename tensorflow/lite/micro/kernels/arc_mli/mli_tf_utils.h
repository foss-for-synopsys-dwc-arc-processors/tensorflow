...
inline void ConvertToMliQuantParamsPerChannel(const TfLiteTensor* tfT,
                                              mli_tensor* mliT, bool is_bias_tensor) {
  ...
  mliT->el_params.sa.dim = is_bias_tensor ? 0 : affine_quantization->quantized_dimension;
  ...
  // //TODO: Maybe later change to real per-axis frac bits
  // for (int i = 0; i < num_channels; i++)
  //   mliT->el_params.sa.scale_frac_bits.mem.pi8[i] = min_frac_bits;

  for (int i = 0; i < num_channels; i++) {
    int16_t iscale = (int16_t)((1ll << mliT->el_params.sa.scale_frac_bits.mem.pi8[i]) * fscale[i] + 0.5f);
...
// datatype* data_ptr = (datatype*)mliT->data.mem.void_p;
  // int32_t new_tensor_offset = mliT->data.capacity;
    ...
    // int mli_tensor_strides_new[MLI_MAX_RANK] = { 0 };
    int shape_idx = mliT->rank - 1;
    int mli_tensor_memstride = 1;
    for (; shape_idx >= 0; --shape_idx) {
            mli_tensor_strides_prev[shape_idx] = (mliT->mem_stride[shape_idx] == 0) ? mli_tensor_memstride : mliT->mem_stride[shape_idx];
            mli_tensor_memstride *= mliT->shape[shape_idx];
    }

    // auto val_pos = [](uint32_t strides[MLI_MAX_RANK], int dim0_idx, int dim1_idx, int dim2_idx, int dim3_idx) -> int {
  ...
  //         uint16_t prev_position = val_pos(mli_tensor_strides_prev, dim0_idx, dim1_idx, dim2_idx, dim3_idx);
  //         uint16_t new_position = val_pos(mliT->mem_stride, dim1_idx, dim2_idx, dim0_idx, dim3_idx) + new_tensor_offset;
  //         data_ptr[new_position] = data_ptr[prev_position];
  ...
  // mliT->data.mem.pi8 += new_tensor_offset;

}

template <typename datatype>
inline void ConvertNHWCToHWCN(mli_tensor* mliT) {
  datatype* data_ptr = (datatype*)mliT->data.mem.void_p;
  int32_t new_tensor_offset = mliT->data.capacity;
  ...
  for (int dim0_idx = dim_start[0]; dim0_idx < dim_end[0]; ++dim0_idx) {
    for (int dim1_idx = dim_start[1]; dim1_idx < dim_end[1]; ++dim1_idx) {
      for (int dim2_idx = dim_start[2]; dim2_idx < dim_end[2]; ++dim2_idx) {
        for (int dim3_idx = dim_start[3]; dim3_idx < dim_end[3]; ++dim3_idx) {
          uint16_t prev_position = val_pos(mli_tensor_strides_prev, dim0_idx, dim1_idx, dim2_idx, dim3_idx);
          uint16_t new_position = val_pos(mliT->mem_stride, dim1_idx, dim2_idx, dim3_idx, dim0_idx) + new_tensor_offset;
          data_ptr[new_position] = data_ptr[prev_position];
          // uint16_t prev_position = ;
        }
      }
