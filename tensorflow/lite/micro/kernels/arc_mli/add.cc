...
TfLiteStatus CalculateOpData(TfLiteContext* context, TfLiteAddParams* params,
  ...
  if (output->type == kTfLiteUInt8 || output->type == kTfLiteInt8) {
    ...
    if (data->is_mli_applicable) {
      data->mli_input1 = ops::micro::MliTensorInterface(static_cast<mli_tensor*>(
          context->AllocatePersistentBuffer(context, sizeof(mli_tensor))));
      data->mli_input2 = ops::micro::MliTensorInterface(static_cast<mli_tensor*>(
          context->AllocatePersistentBuffer(context, sizeof(mli_tensor))));
      ...
      /* Flatten tensors to simplify the process (as we don't support broadcasting). */
...
TfLiteStatus EvalMLIAddInt8(TfLiteContext* context, TfLiteNode* node,
  ...
  TF_LITE_ENSURE_STATUS(
      ops::micro::get_arc_scratch_buffer_for_eltwise_tensors(
          context, &input1_local, &input2_local, &out_local));
  TF_LITE_ENSURE(context, *input1_local.Rank() == 1 && 
                 *input2_local.Rank() == 1 && *out_local.Rank() == 1);
  ...
  ops::micro::TensorSlicer input1_slice(data->mli_input1.MliTensor(),
                                        slice_dim, slice_size);
  ops::micro::TensorSlicer input2_slice(data->mli_input2.MliTensor(),
                                        slice_dim, slice_size);
  ops::micro::TensorSlicer out_slice(data->mli_out.MliTensor(),
                                     slice_dim, slice_size);

  mli_tensor* input1_tsr = input1_is_local ? input1_slice.Sub() 
                                           : input1_local.MliTensor();
  mli_tensor* input2_tsr = input2_is_local ? input2_slice.Sub() 
                                           : input2_local.MliTensor();
  mli_tensor* out_tsr = out_is_local ? out_slice.Sub() 
                                     : out_local.MliTensor();
...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  if (data->is_mli_applicable) {
    ret_val = EvalMLIAddInt8(context, node, params, data,
                             input1, input2, output);
    TF_LITE_ENSURE_OK(context, ret_val);
  } else {
    switch(output->type) {
      case kTfLiteFloat32:
      ...
      case kTfLiteUInt8:
      case kTfLiteInt8:
        ret_val = EvalAddQuantized(context, node, params, data,
                                   input1, input2, output);
        TF_LITE_ENSURE_OK(context, ret_val);
        break;
      default:
